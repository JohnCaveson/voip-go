  ...
  useEffect(() => {
    ...
    client.on('offer', (msg) => {
      if (msg.sdp && msg.sender_id) {
        options.onOffer?.(msg.sender_id, msg.sdp)
      }
    })

    client.on('answer', (msg) => {
      if (msg.sdp && msg.sender_id) {
        options.onAnswer?.(msg.sender_id, msg.sdp)
      }
    })

    client.on('ice_candidate', (msg) => {
      if (msg.candidate && msg.sender_id) {
        options.onICE?.(msg.sender_id, msg.candidate)
      }
    })

    client.on('peer_joined', (msg) => {
      if (msg.sender_id) {
        options.onPeerJoined?.(msg.sender_id)
      }
    })

    client.on('peer_left', (msg) => {
      if (msg.sender_id) {
        options.onPeerLeft?.(msg.sender_id)
      }
    })
    ...
    const connect = async () => {
      try {
        ...
        userIdRef.current = `user-${Date.now()}`
